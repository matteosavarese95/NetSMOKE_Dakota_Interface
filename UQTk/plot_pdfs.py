...
plt.rc('axes', labelsize=0.8*lsize, titlesize=lsize, linewidth=2)
plt.rc('xtick', labelsize=0.8*lsize)
plt.rc('ytick', labelsize=0.8*lsize)
plt.rc('legend', fontsize=lsize, loc='best')
plt.rc('font', size=lsize, family='STIXGeneral')
...
for i in range(npar):
    ...
    for isam_xi in range(nsam_xi):
        plot_pdf1d(samples[isam_xi::nsam_xi, i], pltype='kde', ax=thisax, lw=lwdt)
    ...
    if nominal_file is not None:
        plot_pdf1d(np.array(nominals[ind_show[i]], ndmin=2), pltype='nom',
                   nom_height_factor=1., color='r', ax=thisax, lw=lwdt)
    if show_range:
        ...
        thisax.plot([prange[ind_show[i], 0], prange[ind_show[i], 0],
                    ...
                    [0.0, 1.0 / dr, 1.0 / dr, 0.0], 'g--', linewidth=lwdt, label='Prior', zorder=-100)
    ...
    if plot_type == 'tri':
        ...
        plt.savefig('pdf_' + names[ind_show[i]] + '.png',dpi=300)
        # plt.clf()
    ...
    for j in range(i):
        ...
        if plot_type == 'tri':
            ...
            plt.savefig('pdf_' + names[ind_show[j]] + '_' + names[ind_show[i]] + '.png',dpi=300)
            # plt.clf()


if plot_type == 'tri':
    plt.savefig('pdf_tri.png',dpi=300)
    # plt.show()
