...
lsize = 36
plt.rc('axes', labelsize=0.6*lsize, titlesize=lsize, linewidth=2)
plt.rc('xtick', labelsize=0.8*lsize)
plt.rc('ytick', labelsize=0.8*lsize)
plt.rc('legend', fontsize=0.5*lsize, loc='best')
plt.rc('font', size=0.8*lsize, family='STIXGeneral')
    ...
    for j in range(bcg_data.shape[1]):
        if datastd_file is not None:
            ...
            thisax.errorbar(xdata, bcg_data[:, j], yerr=bcg_data_std[:, j],
                            ecolor='k', fmt='ko', label='CFD Data', ms=12, zorder=50000)
        else:
            thisax.plot(xdata, bcg_data[:, j], 'ko', label='CFD Data', ms=12, zorder=50000)
...
thisax.legend(handles, labels, ncol=1)
#thisax.grid(False)
plt.savefig('fit_vars.png',dpi=800)
